...
// ==========================================================================
// 🎮 AAA戦闘UI全面リデザインパッチ（CSS主導・ロジック完全温存）
//    コンセプト：英単語学習なのに本格RPG／ダークファンタジー×ネオン×ガラス
//    ・#multi-battle-play-screen 配下のみをスコープ化（対人戦も同一IDで恩恵）
//    ・既存ID/classをターゲットに“見た目だけ”を書き換え＝ロジック不変
//    ・足りない装飾（粒子/霧/光線/円形タイマー/コンボバナー/敵名スロット）
//      は aaa- 接頭辞のDOMを最小注入。二重適用ガード＋try/catch で固める
//    ・HP色相(緑→黄→赤)・円形タイマー・ダメージ色分け・コンボ文言は
//      既存関数を“ラップ”して付加（元を必ず呼ぶ＝挙動を壊さない）
//    ※このファイルの末尾にそのまま貼り付けてください
// ==========================================================================
(function applyAAABattleRedesign() {
if (window.__aaaBattleRedesignApplied) return;
window.__aaaBattleRedesignApplied = true;

var FONT_DISPLAY = "'Cinzel','Noto Serif JP',serif";
var FONT_HUD     = "'Chakra Petch','Rajdhani',sans-serif";
var FONT_BODY    = "'Noto Sans JP',system-ui,sans-serif";
...
l.href = 'https://fonts.googleapis.com/css2?family=Cinzel:wght@500;700;900&family=Chakra+Petch:wght@500;600;700&family=Noto+Serif+JP:wght@700;900&family=Noto+Sans+JP:wght@500;700;900&display=swap';
document.head.appendChild(l);
})();

// ------------------------------------------------------------------
// 【1】AAAスタイル注入（戦闘画面スコープ＋レイヤー＋全要素）
// ------------------------------------------------------------------
(function injectAAAStyle() {
if (document.getElementById('aaaBattleStyle')) return;
var s = document.createElement('style');
s.id = 'aaaBattleStyle';
s.textContent = [
/* ===== 設計トークン ===== */
':root{',
'--aaa-bg-0:#06040f;--aaa-bg-1:#0c0820;--aaa-bg-2:#150d31;',
'--aaa-cyan:#34e7e4;--aaa-cyan-soft:#9af6f1;',
'--aaa-emerald:#3ddc97;--aaa-gold:#f5c451;--aaa-gold-deep:#c8902a;',
'--aaa-danger:#ff5468;--aaa-orange:#ff8a3d;',
'--aaa-violet:#9b6bff;--aaa-violet-deep:#5b2da8;',
'--aaa-glass:rgba(18,13,38,.52);--aaa-glass-2:rgba(28,20,54,.62);',
'--aaa-line:rgba(155,107,255,.30);--aaa-line-cyan:rgba(52,231,228,.34);',
'--aaa-ink:#eef0ff;--aaa-ink-dim:#a9a4d6;',
'}',

/* ===== 戦闘画面：背景レイヤー強化 ===== */
'body.in-game-active #multi-battle-play-screen{',
'position:relative;overflow:hidden;',
'background:radial-gradient(120% 90% at 50% -10%, #1a1140 0%, var(--aaa-bg-1) 42%, var(--aaa-bg-0) 100%) !important;',
'}',
/* 既存背景画像を“奥”へ沈める暗いオーバーレイ */
'body.in-game-active #multi-battle-play-bg{',
'filter:blur(3px) brightness(.42) saturate(1.15) contrast(1.08) !important;',
'opacity:.9 !important;',
'}',
'body.in-game-active #multi-battle-play-screen::before{', /* ビネット＋色かぶり */
'content:"";position:fixed;inset:0;z-index:1;pointer-events:none;',
'background:',
'radial-gradient(140% 120% at 50% 30%, transparent 38%, rgba(6,4,15,.55) 78%, rgba(6,4,15,.92) 100%),',
'linear-gradient(180deg, rgba(91,45,168,.20) 0%, transparent 30%, transparent 62%, rgba(6,4,15,.85) 100%);',
'mix-blend-mode:normal;',
'}',
'body.in-game-active #multi-battle-play-screen::after{', /* 走査線＋ノイズ感 */
'content:"";position:fixed;inset:0;z-index:1;pointer-events:none;opacity:.5;',
'background:repeating-linear-gradient(0deg, rgba(255,255,255,.018) 0 1px, transparent 1px 3px);',
'mix-blend-mode:overlay;',
'}',
/* 浮遊パーティクル／霧／光線のコンテナ（JSが中身を生成） */
'#aaaBattleFx{position:fixed;inset:0;z-index:1;pointer-events:none;overflow:hidden;}',
'#aaaBattleFx .aaa-fog{position:absolute;inset:-20%;opacity:.5;mix-blend-mode:screen;',
'background:',
'radial-gradient(40% 30% at 20% 30%, rgba(91,45,168,.40), transparent 70%),',
'radial-gradient(35% 28% at 80% 65%, rgba(52,231,228,.22), transparent 72%);',
'animation:aaaFogDrift 26s ease-in-out infinite alternate;filter:blur(8px);}',
'#aaaBattleFx .aaa-fog.two{animation-duration:34s;animation-direction:alternate-reverse;opacity:.35;',
'background:radial-gradient(45% 32% at 65% 20%, rgba(245,196,81,.16), transparent 70%);}',
'@keyframes aaaFogDrift{0%{transform:translate3d(-4%,-2%,0) scale(1.05)}100%{transform:translate3d(5%,3%,0) scale(1.18)}}',
'#aaaBattleFx .aaa-ray{position:absolute;top:-30%;left:50%;width:160%;height:160%;',
'transform-origin:50% 50%;opacity:.10;mix-blend-mode:screen;',
'background:conic-gradient(from 0deg, transparent 0 8deg, rgba(155,107,255,.5) 9deg 11deg, transparent 12deg 30deg, rgba(52,231,228,.4) 31deg 33deg, transparent 34deg 70deg, rgba(245,196,81,.35) 71deg 73deg, transparent 74deg 360deg);',
'animation:aaaRaySpin 60s linear infinite;filter:blur(2px);}',
'@keyframes aaaRaySpin{to{transform:translate(-50%,-50%) rotate(360deg)}}',
'#aaaBattleFx .aaa-mote{position:absolute;bottom:-12px;border-radius:50%;pointer-events:none;',
'background:radial-gradient(circle, var(--mc,rgba(155,107,255,.9)), transparent 70%);',
'animation:aaaMoteRise linear infinite;filter:blur(.3px);}',
'@keyframes aaaMoteRise{0%{transform:translateY(0) translateX(0) scale(.5);opacity:0}12%{opacity:.9}82%{opacity:.55}100%{transform:translateY(-104vh) translateX(var(--mx,20px)) scale(1.1);opacity:0}}',

/* 戦闘コンテンツを背景より手前へ */
'body.in-game-active #multi-battle-play-screen > *:not(#multi-battle-play-bg):not(#aaaBattleFx){position:relative;z-index:3;}',

/* ===== 敵エリア ===== */
'#multiBossHpBarContainer{position:relative;}',
/* 敵画像：大きく＋レアリティ別オーラ（既定はBOSS＝金紫） */
'#multiBossImage{',
'width:auto !important;max-width:74vw !important;height:auto !important;max-height:42vh !important;',
'object-fit:contain !important;',
'filter:drop-shadow(0 14px 30px rgba(0,0,0,.7)) drop-shadow(0 0 26px rgba(155,107,255,.45)) drop-shadow(0 0 60px rgba(91,45,168,.35)) !important;',
'animation:aaaBossFloat 5.5s ease-in-out infinite !important;',
'transition:filter .25s ease, transform .12s ease;',
'}',
'@keyframes aaaBossFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-10px) scale(1.015)}}',
'#multiBossImage.aaa-hit{animation:aaaBossHit .42s cubic-bezier(.36,.07,.19,.97) !important;}',
'@keyframes aaaBossHit{0%{transform:translateX(0)}15%{transform:translateX(-9px) rotate(-1.5deg)}30%{transform:translateX(8px) rotate(1.5deg)}45%{transform:translateX(-6px)}60%{transform:translateX(5px)}80%{transform:translateX(-2px)}100%{transform:translateX(0)}}',
/* 敵の足元の影＋魔力陣 */
'#aaaEnemyStage{position:relative;display:flex;flex-direction:column;align-items:center;}',
'#aaaEnemyStage::after{content:"";position:absolute;left:50%;bottom:6%;width:62%;height:26px;transform:translateX(-50%);',
'background:radial-gradient(50% 100% at 50% 50%, rgba(155,107,255,.5), rgba(52,231,228,.18) 45%, transparent 72%);',
'filter:blur(5px);border-radius:50%;animation:aaaStagePulse 4s ease-in-out infinite;}',
'@keyframes aaaStagePulse{0%,100%{opacity:.5;transform:translateX(-50%) scaleX(1)}50%{opacity:.85;transform:translateX(-50%) scaleX(1.12)}}',

/* 敵名＋レアリティバッジ */
'#aaaEnemyNameWrap{display:flex;flex-direction:column;align-items:center;gap:7px;margin-bottom:10px;}',
'#aaaEnemyName{font-family:var(--aaa-display);font-weight:900;font-size:clamp(20px,6.4vw,30px);',
'letter-spacing:.14em;color:#fff;text-transform:uppercase;line-height:1;',
'text-shadow:0 0 18px rgba(155,107,255,.6),0 2px 4px rgba(0,0,0,.8);text-align:center;}',
'#aaaRarityBadge{position:relative;font-family:var(--aaa-hud);font-weight:700;font-size:11px;letter-spacing:.34em;',
'padding:5px 18px 5px 22px;color:#1a1206;text-transform:uppercase;',
'background:linear-gradient(180deg,#ffe9a8,#f5c451 46%,#c8902a);',
'border:1px solid rgba(255,233,168,.85);border-radius:3px;',
'box-shadow:0 0 16px rgba(245,196,81,.55),inset 0 1px 0 rgba(255,255,255,.7),inset 0 -2px 4px rgba(120,70,10,.5);',
'clip-path:polygon(8px 0,100% 0,calc(100% - 8px) 100%,0 100%);}',
'#aaaRarityBadge::before{content:"";position:absolute;inset:0;background:linear-gradient(110deg,transparent 30%,rgba(255,255,255,.85) 48%,transparent 62%);',
'animation:aaaBadgeSheen 3.4s ease-in-out infinite;mix-blend-mode:overlay;}',
'@keyframes aaaBadgeSheen{0%,60%,100%{transform:translateX(-120%)}80%{transform:translateX(120%)}}',
/* レアリティ別カラー（将来 .enemy-rarity-X を付与すれば自動切替） */
'#aaaRarityBadge.r-C{background:linear-gradient(180deg,#e8ecf5,#aab2c5 50%,#6b7governs);background:linear-gradient(180deg,#e8ecf5,#aab2c5 50%,#6b7488);color:#10131c;box-shadow:0 0 12px rgba(170,178,197,.5),inset 0 1px 0 rgba(255,255,255,.7);border-color:rgba(232,236,245,.8);}',
'#aaaRarityBadge.r-UC{background:linear-gradient(180deg,#c8fffb,#34e7e4 50%,#0e8f8c);color:#04201f;box-shadow:0 0 14px rgba(52,231,228,.55),inset 0 1px 0 rgba(255,255,255,.7);border-color:rgba(200,255,251,.8);}',
'#aaaRarityBadge.r-R{background:linear-gradient(180deg,#ffd9b0,#ff8a3d 50%,#b8480f);color:#2a0f02;box-shadow:0 0 16px rgba(255,138,61,.55),inset 0 1px 0 rgba(255,255,255,.7);border-color:rgba(255,217,176,.85);}',
'#aaaRarityBadge.r-SR{background:linear-gradient(180deg,#ffe9a8,#f5c451 46%,#c8902a);color:#1a1206;box-shadow:0 0 18px rgba(245,196,81,.6),inset 0 1px 0 rgba(255,255,255,.7);border-color:rgba(255,233,168,.85);}',

/* ===== ボスHPバー：豪華化＋色相＋シマー ===== */
'.multi-boss-full-bar{',
'height:26px !important;border-radius:7px !important;overflow:hidden !important;',
'background:linear-gradient(180deg,#1a0c12,#0a0509) !important;',
'border:1.5px solid transparent !important;',
'background-clip:padding-box !important;',
'box-shadow:0 0 0 1.5px rgba(245,196,81,.45),0 0 22px rgba(255,84,104,.30),inset 0 2px 6px rgba(0,0,0,.85),inset 0 -1px 0 rgba(255,255,255,.06) !important;',
'position:relative;',
'}',
'.multi-boss-full-bar::before{content:"";position:absolute;inset:0;border-radius:6px;pointer-events:none;z-index:4;',
'box-shadow:inset 0 1px 0 rgba(255,255,255,.10);}',
'.multi-boss-hp-fill{',
'background:linear-gradient(180deg,',
'  hsl(calc(var(--hp-ratio,1)*120),92%,66%) 0%,',
'  hsl(calc(var(--hp-ratio,1)*120),88%,48%) 48%,',
'  hsl(calc(var(--hp-ratio,1)*120),82%,34%) 100%) !important;',
'box-shadow:0 0 14px hsla(calc(var(--hp-ratio,1)*120),90%,55%,.6),inset 0 1px 0 rgba(255,255,255,.45),inset 0 -3px 6px rgba(0,0,0,.4) !important;',
'transition:width .45s cubic-bezier(.22,1,.36,1), background .4s ease !important;',
'border-radius:5px !important;',
'}',
/* 流れる光沢 */
'.multi-boss-hp-fill::after{content:"";position:absolute;top:0;left:-60%;width:46%;height:100%;',
'background:linear-gradient(100deg,transparent,rgba(255,255,255,.55),transparent);transform:skewX(-18deg);',
'animation:aaaHpSheen 2.6s ease-in-out infinite;pointer-events:none;}',
'@keyframes aaaHpSheen{0%{left:-60%}55%,100%{left:130%}}',
/* ダメージ時のバー揺れ */
'.multi-boss-full-bar.aaa-bar-shake{animation:aaaBarShake .4s ease;}',
'@keyframes aaaBarShake{0%,100%{transform:translateX(0)}20%{transform:translateX(-4px)}40%{transform:translateX(4px)}60%{transform:translateX(-3px)}80%{transform:translateX(2px)}}',
'.multi-boss-hp-text-layer{z-index:5 !important;}',
'#multiEnemyHpText{font-family:var(--aaa-hud) !important;font-weight:700 !important;font-size:13px !important;',
'letter-spacing:.06em;color:#fff !important;text-shadow:0 1px 3px rgba(0,0,0,.95),0 0 8px rgba(0,0,0,.7) !important;}',

/* ===== 円形タイマー ===== */
'#aaaTimerRing{position:absolute;top:50%;right:-6px;transform:translate(108%,-50%);width:62px;height:62px;z-index:6;',
'filter:drop-shadow(0 4px 12px rgba(0,0,0,.6));}',
'#aaaTimerRing svg{transform:rotate(-90deg);overflow:visible;}',
'#aaaTimerRing .aaa-tr-bg{fill:rgba(8,5,18,.78);stroke:rgba(155,107,255,.22);stroke-width:2;}',
'#aaaTimerRing .aaa-tr-fg{fill:none;stroke:url(#aaaTrGrad);stroke-width:4;stroke-linecap:round;',
'transition:stroke-dashoffset .18s linear;filter:drop-shadow(0 0 5px rgba(255,84,104,.7));}',
'#aaaTimerRing .aaa-tr-num{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;',
'font-family:var(--aaa-hud);font-weight:700;font-size:17px;color:#fff;letter-spacing:.02em;',
'text-shadow:0 0 10px rgba(255,84,104,.6),0 1px 2px #000;}',
'#aaaTimerRing.aaa-urgent .aaa-tr-num{color:var(--aaa-danger);animation:aaaUrgentPulse .6s ease-in-out infinite;}',
'@keyframes aaaUrgentPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.16)}}',
'#aaaTimerRing .aaa-tr-lbl{position:absolute;left:50%;top:-15px;transform:translateX(-50%);',
'font-family:var(--aaa-hud);font-size:8px;font-weight:600;letter-spacing:.22em;color:var(--aaa-ink-dim);white-space:nowrap;}',
/* 既存のテキストタイマーは円形で代替するので隠す（円生成失敗時は残す保険） */
'#aaaTimerRing.ok ~ #multiEnemyTimerDisplay{display:none !important;}',
'#multiEnemyTimerDisplay{font-family:var(--aaa-hud) !important;font-weight:700 !important;',
'background:rgba(8,5,18,.6);border:1px solid var(--aaa-line);border-radius:20px;padding:3px 12px !important;',
'color:var(--aaa-danger) !important;text-shadow:0 0 8px rgba(255,84,104,.5);backdrop-filter:blur(6px);}',

/* ===== パーティ／プレイヤーカード ===== */
'.multi-party-status-area{',
'background:linear-gradient(180deg,var(--aaa-glass-2),var(--aaa-glass)) !important;',
'border:1px solid var(--aaa-line) !important;border-radius:16px !important;',
'box-shadow:0 10px 34px rgba(0,0,0,.5),inset 0 1px 0 rgba(255,255,255,.08),inset 0 0 30px rgba(91,45,168,.12) !important;',
'backdrop-filter:blur(14px) !important;-webkit-backdrop-filter:blur(14px) !important;',
'padding:12px 10px !important;',
'}',
'.multi-party-member{position:relative;}',
/* アイコンを豪華フレーム化（六角風クリップ＋発光リング） */
'.multi-party-icon{',
'width:54px !important;height:54px !important;border-radius:14px !important;overflow:hidden !important;',
'background:radial-gradient(circle at 50% 35%, rgba(155,107,255,.25), rgba(8,5,18,.85)) !important;',
'border:1.5px solid rgba(155,107,255,.5) !important;',
'box-shadow:0 0 0 2px rgba(8,5,18,.9),0 0 16px rgba(155,107,255,.4),inset 0 2px 6px rgba(0,0,0,.7) !important;',
'clip-path:polygon(50% 0,100% 25%,100% 75%,50% 100%,0 75%,0 25%) !important;',
'transition:box-shadow .25s ease, transform .2s ease;',
'}',
'.multi-party-member:first-child .multi-party-icon{', /* 自分＝シアン発光 */
'border-color:rgba(52,231,228,.7) !important;box-shadow:0 0 0 2px rgba(8,5,18,.9),0 0 18px rgba(52,231,228,.55),inset 0 2px 6px rgba(0,0,0,.7) !important;',
'}',
'.multi-party-icon img{width:100%;height:100%;object-fit:cover;}',
/* 仮シルエットを“高品質プレースホルダ”へ（青単色を脱却） */
'.multi-party-icon:not(:has(img))::after{content:"";position:absolute;inset:0;',
'background:radial-gradient(circle at 50% 38%, rgba(155,107,255,.35), transparent 60%),linear-gradient(180deg,rgba(155,107,255,.12),rgba(8,5,18,.4));}',
/* 名前＋Lvチップ */
'.multi-party-member > div[style*="font-size:8px"]{',
'font-family:var(--aaa-hud) !important;font-size:9px !important;font-weight:600 !important;',
'letter-spacing:.04em;color:var(--aaa-cyan-soft) !important;text-shadow:0 0 8px rgba(52,231,228,.4);}',
'.multi-party-member:first-child > div[style*="font-size:8px"]{color:var(--aaa-cyan) !important;}',
/* 装備アイコンをガラスチップ化 */
'.multi-party-equip-display{background:rgba(8,5,18,.55) !important;border:1px solid rgba(255,255,255,.12) !important;',
'border-radius:8px !important;backdrop-filter:blur(4px);box-shadow:inset 0 1px 0 rgba(255,255,255,.08);}',
/* パーティHPバー：細いネオン */
'.multi-party-hp-bar{height:6px !important;border-radius:4px !important;background:rgba(0,0,0,.7) !important;',
'border:1px solid rgba(255,255,255,.12) !important;box-shadow:inset 0 1px 3px rgba(0,0,0,.8) !important;}',
'.multi-party-hp-fill{background:linear-gradient(90deg,#1f9d6b,#3ddc97 60%,#9af6d0) !important;',
'box-shadow:0 0 8px rgba(61,220,151,.6),inset 0 1px 0 rgba(255,255,255,.4) !important;position:relative;overflow:hidden;}',
'.multi-party-hp-fill::after{content:"";position:absolute;top:0;left:-60%;width:50%;height:100%;',
'background:linear-gradient(100deg,transparent,rgba(255,255,255,.5),transparent);transform:skewX(-18deg);animation:aaaHpSheen 3s ease-in-out infinite;}',
/* コンボテキスト */
'.multi-party-combo{font-family:var(--aaa-hud) !important;font-weight:700 !important;color:var(--aaa-gold) !important;',
'text-shadow:0 0 10px rgba(245,196,81,.7),0 1px 2px #000 !important;letter-spacing:.05em;}',

/* ===== 自分HP／リミットゲージのパネル ===== */
'#multiPlayerOwnHpFrame{',
'background:linear-gradient(180deg,var(--aaa-glass-2),var(--aaa-glass)) !important;',
'border:1px solid var(--aaa-line-cyan) !important;border-radius:14px !important;',
'box-shadow:0 8px 26px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.08),inset 0 0 24px rgba(52,231,228,.08) !important;',
'backdrop-filter:blur(12px) !important;-webkit-backdrop-filter:blur(12px) !important;padding:10px 12px !important;',
'}',
'.multi-hp-bar{border-radius:8px !important;border:1px solid rgba(255,255,255,.14) !important;',
'background:rgba(0,0,0,.6) !important;box-shadow:inset 0 2px 5px rgba(0,0,0,.8) !important;overflow:hidden;}',
'.multi-party-hp-fill[id^="multiPlayerOwnHpFill"],#multiPlayerOwnHpFill{',
'background:linear-gradient(90deg,#1f9d6b,#3ddc97 55%,#9af6d0) !important;box-shadow:0 0 10px rgba(61,220,151,.55),inset 0 1px 0 rgba(255,255,255,.4) !important;}',
'.multi-limit-fill{box-shadow:0 0 12px rgba(245,196,81,.7),inset 0 1px 0 rgba(255,255,255,.4) !important;}',
'.multi-limit-fill.max{animation:aaaLimitMax 1.4s linear infinite !important;box-shadow:0 0 22px rgba(255,255,255,.85) !important;}',
'@keyframes aaaLimitMax{0%{background-position:0 50%}100%{background-position:200% 50%}}',
'.hp-bar-inner-text-layer{font-family:var(--aaa-hud) !important;font-weight:700 !important;letter-spacing:.04em;}',

/* ===== TARGET WORD：主役化 ===== */
'.multi-question-header-panel{margin:6px 0 4px !important;}',
'.multi-quest-lbl{font-family:var(--aaa-hud) !important;font-size:10px !important;letter-spacing:.42em !important;',
'color:var(--aaa-cyan) !important;text-shadow:0 0 12px rgba(52,231,228,.6);text-transform:uppercase;}',
'#flickTargetWord{',
'display:inline-block;font-family:var(--aaa-display) !important;font-weight:900 !important;',
'font-size:clamp(30px,9.5vw,46px) !important;letter-spacing:.02em;line-height:1.05;color:#fff !important;',
'padding:14px 30px !important;border-radius:18px !important;',
'background:linear-gradient(180deg,rgba(28,20,54,.72),rgba(12,8,32,.78)) !important;',
'border:1.5px solid var(--aaa-line-cyan) !important;',
'box-shadow:0 0 0 1px rgba(52,231,228,.18),0 14px 40px rgba(0,0,0,.55),0 0 34px rgba(52,231,228,.30),inset 0 1px 0 rgba(255,255,255,.12),inset 0 0 30px rgba(52,231,228,.10) !important;',
'backdrop-filter:blur(10px) !important;-webkit-backdrop-filter:blur(10px) !important;',
'text-shadow:0 0 22px rgba(52,231,228,.55),0 2px 4px rgba(0,0,0,.85) !important;',
'position:relative;overflow:hidden;animation:aaaWordIn .5s cubic-bezier(.2,.9,.3,1.2);}',
'@keyframes aaaWordIn{from{opacity:0;transform:translateY(10px) scale(.94)}to{opacity:1;transform:none}}',
'#flickTargetWord::before{content:"";position:absolute;top:0;left:-60%;width:50%;height:100%;',
'background:linear-gradient(100deg,transparent,rgba(255,255,255,.16),transparent);transform:skewX(-18deg);animation:aaaHpSheen 4s ease-in-out infinite;}',

/* ===== 選択肢：カードUI ===== */
'.multi-flick-area{',
'background:linear-gradient(180deg,rgba(18,13,38,.40),rgba(8,5,18,.55)) !important;',
'border:1px solid var(--aaa-line) !important;border-radius:20px !important;',
'box-shadow:0 12px 40px rgba(0,0,0,.5),inset 0 1px 0 rgba(255,255,255,.06),inset 0 0 40px rgba(91,45,168,.10) !important;',
'backdrop-filter:blur(12px) !important;-webkit-backdrop-filter:blur(12px) !important;padding:14px 12px 16px !important;',
'}',
'.multi-flick-area::before{background-image:linear-gradient(rgba(155,107,255,.06) 1px,transparent 1px),linear-gradient(90deg,rgba(155,107,255,.06) 1px,transparent 1px) !important;background-size:26px 26px !important;}',
'.multi-grid-3x3{gap:8px !important;max-width:330px !important;}',
'.flick-choice{',
'min-height:52px !important;border-radius:13px !important;padding:8px 6px !important;',
'font-family:var(--aaa-body) !important;font-size:13px !important;font-weight:700 !important;line-height:1.25 !important;',
'color:var(--aaa-ink) !important;',
'background:linear-gradient(180deg,rgba(40,30,72,.66),rgba(20,14,42,.78)) !important;',
'border:1px solid rgba(155,107,255,.26) !important;',
'box-shadow:0 4px 14px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.08) !important;',
'text-shadow:0 1px 3px rgba(0,0,0,.8) !important;',
'transition:transform .14s cubic-bezier(.2,.9,.3,1.3),box-shadow .2s ease,border-color .2s ease,background .2s ease !important;',
'position:relative;overflow:hidden;',
'}',
'.flick-choice::after{content:"";position:absolute;inset:0;border-radius:12px;opacity:0;transition:opacity .2s;',
'background:radial-gradient(circle at 50% 0%,rgba(52,231,228,.25),transparent 70%);}',
'.flick-choice.highlight{',
'transform:scale(1.07) !important;z-index:20 !important;',
'border-color:var(--aaa-cyan) !important;color:#fff !important;',
'background:linear-gradient(180deg,rgba(52,231,228,.30),rgba(14,143,140,.42)) !important;',
'box-shadow:0 0 26px rgba(52,231,228,.7),inset 0 0 16px rgba(52,231,228,.35),0 6px 18px rgba(0,0,0,.5) !important;',
'text-shadow:0 0 12px rgba(52,231,228,.8) !important;}',
'.flick-choice.highlight::after{opacity:1;}',
/* 正解／不正解の瞬間フィードバック（JSが付与） */
'.flick-choice.aaa-correct{border-color:var(--aaa-emerald) !important;',
'background:linear-gradient(180deg,rgba(61,220,151,.4),rgba(31,157,107,.5)) !important;',
'box-shadow:0 0 30px rgba(61,220,151,.8),inset 0 0 18px rgba(61,220,151,.4) !important;animation:aaaCorrectPop .4s ease;}',
'@keyframes aaaCorrectPop{0%{transform:scale(1)}40%{transform:scale(1.12)}100%{transform:scale(1)}}',
'.flick-choice.aaa-wrong{border-color:var(--aaa-danger) !important;',
'background:linear-gradient(180deg,rgba(255,84,104,.34),rgba(150,20,40,.5)) !important;',
'box-shadow:0 0 28px rgba(255,84,104,.75),inset 0 0 16px rgba(255,84,104,.4) !important;animation:aaaWrongShake .42s ease;}',
'@keyframes aaaWrongShake{0%,100%{transform:translateX(0)}20%{transform:translateX(-6px)}40%{transform:translateX(6px)}60%{transform:translateX(-4px)}80%{transform:translateX(3px)}}',
'.flick-choice-center-blank{min-height:52px !important;}',
/* 中央の武器アイコンを魔力コア化 */
'.flick-weapon-icon{font-size:30px !important;filter:drop-shadow(0 0 12px rgba(255,138,61,.9)) drop-shadow(0 0 24px rgba(245,196,81,.6)) !important;',
'animation:aaaCorePulse 2.2s ease-in-out infinite !important;}',
'@keyframes aaaCorePulse{0%,100%{transform:translate(-50%,-50%) scale(1)}50%{transform:translate(-50%,-50%) scale(1.18)}}',

/* ===== 逃げる／降参ボタン ===== */
'#multiEscapeOrSurrenderBtn,#multi-battle-play-screen [id*="Escape"],#multi-battle-play-screen [id*="escape"]{',
'font-family:var(--aaa-hud) !important;font-weight:600 !important;letter-spacing:.12em !important;',
'background:linear-gradient(180deg,rgba(40,30,72,.7),rgba(20,14,42,.8)) !important;',
'border:1px solid rgba(255,84,104,.4) !important;color:#ffd2d8 !important;border-radius:10px !important;',
'box-shadow:0 4px 14px rgba(0,0,0,.4),inset 0 1px 0 rgba(255,255,255,.08) !important;',
'backdrop-filter:blur(8px);text-shadow:0 1px 3px rgba(0,0,0,.8);transition:all .2s ease;',
'padding:8px 16px !important;}',
'#multiEscapeOrSurrenderBtn:active{transform:scale(.94);box-shadow:0 0 18px rgba(255,84,104,.5) !important;}',

/* ===== ダメージ／攻撃ポップアップ ===== */
'.popup-v-dmg .v-dmg-text{font-family:var(--aaa-hud) !important;font-weight:700 !important;font-size:20px !important;',
'color:var(--aaa-danger) !important;text-shadow:0 0 12px rgba(255,84,104,.8),2px 2px 0 #fff,-1px -1px 0 #fff,1px -1px 0 #fff,-1px 1px 0 #fff !important;}',
'.popup-v-dmg .v-mark{border-top-color:var(--aaa-danger) !important;filter:drop-shadow(0 0 6px rgba(255,84,104,.9)) !important;}',
'.popup-bubble-flying-atk{font-family:var(--aaa-hud) !important;font-weight:700 !important;',
'background:linear-gradient(180deg,#fff,#ffe9a8) !important;color:#7a2a06 !important;',
'border:2px solid var(--aaa-gold) !important;box-shadow:0 4px 14px rgba(245,196,81,.6),0 0 18px rgba(245,196,81,.5) !important;}',
'.popup-bubble-flying-atk.aaa-crit{background:linear-gradient(180deg,#fff5d6,#f5c451) !important;border-color:#fff !important;',
'box-shadow:0 0 22px rgba(245,196,81,.95),0 0 40px rgba(255,255,255,.5) !important;transform:translate(-50%,-50%) scale(1.25);}',
'.popup-hit-explosion{background:radial-gradient(circle,#fff 6%,#f5c451 26%,#ff5468 56%,transparent 72%) !important;}',

/* ===== コンボバナー ===== */
'#aaaComboBanner{position:fixed;top:14%;left:50%;transform:translate(-50%,-12px) scale(.8);z-index:8;pointer-events:none;',
'text-align:center;opacity:0;transition:opacity .25s ease,transform .35s cubic-bezier(.2,1.4,.4,1);}',
'#aaaComboBanner.show{opacity:1;transform:translate(-50%,0) scale(1);}',
'#aaaComboBanner .aaa-cb-word{font-family:var(--aaa-display);font-weight:900;font-size:clamp(26px,8vw,40px);',
'letter-spacing:.1em;line-height:1;text-transform:uppercase;',
'background:linear-gradient(180deg,#fff,#f5c451 60%,#ff8a3d);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;',
'filter:drop-shadow(0 0 18px rgba(245,196,81,.7)) drop-shadow(0 3px 5px rgba(0,0,0,.8));}',
'#aaaComboBanner .aaa-cb-num{font-family:var(--aaa-hud);font-weight:700;font-size:clamp(15px,4.5vw,20px);',
'color:var(--aaa-cyan-soft);letter-spacing:.18em;text-shadow:0 0 14px rgba(52,231,228,.8),0 2px 3px #000;margin-top:2px;}',
'#aaaComboBanner.tier-2 .aaa-cb-word{background:linear-gradient(180deg,#fff,#9af6f1 60%,#34e7e4);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 16px rgba(52,231,228,.7)) drop-shadow(0 3px 5px rgba(0,0,0,.8));}',
'#aaaComboBanner.tier-3 .aaa-cb-word{background:linear-gradient(180deg,#fff,#d9c2ff 55%,#9b6bff);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 20px rgba(155,107,255,.8)) drop-shadow(0 3px 5px rgba(0,0,0,.8));}',
'#aaaComboBanner.tier-4 .aaa-cb-word{animation:aaaTier4Glow 1s ease-in-out infinite alternate;}',
'@keyframes aaaTier4Glow{from{filter:drop-shadow(0 0 18px rgba(245,196,81,.7)) drop-shadow(0 3px 5px rgba(0,0,0,.8))}to{filter:drop-shadow(0 0 34px rgba(255,138,61,.95)) drop-shadow(0 3px 5px rgba(0,0,0,.8))}}',
/* コンボ時の画面端フラッシュ */
'#aaaComboFlash{position:fixed;inset:0;z-index:7;pointer-events:none;opacity:0;transition:opacity .3s;',
'box-shadow:inset 0 0 80px rgba(245,196,81,.0);}',
'#aaaComboFlash.tier-1{opacity:1;box-shadow:inset 0 0 90px rgba(52,231,228,.18);}',
'#aaaComboFlash.tier-2{opacity:1;box-shadow:inset 0 0 110px rgba(155,107,255,.24);}',
'#aaaComboFlash.tier-3{opacity:1;box-shadow:inset 0 0 130px rgba(245,196,81,.30);}',
'#aaaComboFlash.tier-4{opacity:1;box-shadow:inset 0 0 150px rgba(255,138,61,.40);animation:aaaFlashPulse 1.1s ease-in-out infinite alternate;}',
'@keyframes aaaFlashPulse{from{box-shadow:inset 0 0 120px rgba(255,138,61,.30)}to{box-shadow:inset 0 0 170px rgba(255,138,61,.5)}}',

/* ===== レスポンシブ＋SafeArea＋アクセシビリティ ===== */
'body.in-game-active .container{padding-left:max(10px,env(safe-area-inset-left)) !important;padding-right:max(10px,env(safe-area-inset-right)) !important;}',
'@media (max-width:380px){',
'#flickTargetWord{font-size:clamp(26px,8.5vw,34px) !important;padding:11px 20px !important;}',
'.flick-choice{min-height:46px !important;font-size:12px !important;}',
'.multi-party-icon{width:46px !important;height:46px !important;}',
'#aaaTimerRing{width:52px;height:52px;transform:translate(112%,-50%);}',
'}',
'@media (max-height:640px){',
'#multiBossImage{max-height:34vh !important;}',
'#flickTargetWord{padding:10px 22px !important;}',
'}',
'@media (prefers-reduced-motion:reduce){',
'#multiBossImage,#aaaBattleFx *,#flickTargetWord::before,.multi-boss-hp-fill::after,.multi-party-hp-fill::after,#aaaRarityBadge::before{animation:none !important;}',
'}'
].join('\n');
document.head.appendChild(s);
})();
...
// ------------------------------------------------------------------
// 【2】背景Fx／敵ステージ／敵名／円形タイマー／コンボバナー のDOM注入
// ------------------------------------------------------------------
function aaaEnsureFx() {
if (!document.getElementById('aaaBattleFx')) {
var fx = document.createElement('div'); fx.id = 'aaaBattleFx';
fx.innerHTML = '<div class="aaa-ray"></div><div class="aaa-fog"></div><div class="aaa-fog two"></div>';
var moteHost = fx;
for (var i = 0; i < 20; i++) {
var m = document.createElement('span'); m.className = 'aaa-mote';
var sz = (2 + Math.round(Math.random() * 4));
var cols = ['rgba(155,107,255,.9)','rgba(52,231,228,.85)','rgba(245,196,81,.8)'];
m.style.width = sz + 'px'; m.style.height = sz + 'px';
m.style.left = Math.round(Math.random() * 100) + '%';
m.style.setProperty('--mc', cols[i % 3]);
m.style.setProperty('--mx', (Math.random() * 60 - 30) + 'px');
m.style.animationDelay = (Math.random() * 12).toFixed(2) + 's';
m.style.animationDuration = (9 + Math.random() * 10).toFixed(2) + 's';
moteHost.appendChild(m);
}
(document.body || document.documentElement).appendChild(fx);
}
if (!document.getElementById('aaaComboBanner')) {
var cb = document.createElement('div'); cb.id = 'aaaComboBanner';
cb.innerHTML = '<div class="aaa-cb-word"></div><div class="aaa-cb-num"></div>';
document.body.appendChild(cb);
}
if (!document.getElementById('aaaComboFlash')) {
var cf = document.createElement('div'); cf.id = 'aaaComboFlash';
document.body.appendChild(cf);
}
}
function aaaBuildEnemyStage() {
var img = document.getElementById('multiBossImage');
if (!img || img.dataset.aaaStaged === '1') return;
img.dataset.aaaStaged = '1';
var stage = document.createElement('div'); stage.id = 'aaaEnemyStage';
img.parentNode.insertBefore(stage, img);
stage.appendChild(img);
// 敵名＋レアリティバッジ（データが無い今はBOSS固定。window.__aaaSetEnemy で上書き可）
var wrap = document.createElement('div'); wrap.id = 'aaaEnemyNameWrap';
wrap.innerHTML = '<div id="aaaRarityBadge" class="r-SR">BOSS</div><div id="aaaEnemyName">ANCIENT ONE</div>';
stage.parentNode.insertBefore(wrap, stage);
}
// 外部から敵名/レアリティを流し込めるフック（将来の敵テーブル連携用）
window.__aaaSetEnemy = function(name, rarity) {
var n = document.getElementById('aaaEnemyName');
var b = document.getElementById('aaaRarityBadge');
if (n && name) n.textContent = String(name);
if (b && rarity) { b.textContent = String(rarity).toUpperCase(); b.className = 'r-' + String(rarity).toUpperCase(); }
};
function aaaBuildTimerRing() {
var bar = document.getElementById('multiBossHpBarContainer');
if (!bar || document.getElementById('aaaTimerRing')) return;
var ring = document.createElement('div'); ring.id = 'aaaTimerRing';
ring.innerHTML =
'<span class="aaa-tr-lbl">ACTION</span>' +
'<svg width="62" height="62" viewBox="0 0 62 62">' +
'<defs><linearGradient id="aaaTrGrad" x1="0" y1="0" x2="1" y2="1">' +
'<stop offset="0" stop-color="#ff8a3d"/><stop offset="1" stop-color="#ff5468"/></linearGradient></defs>' +
'<circle class="aaa-tr-bg" cx="31" cy="31" r="27"/>' +
'<circle class="aaa-tr-fg" cx="31" cy="31" r="27"/>' +
'</svg><div class="aaa-tr-num">10</div>';
bar.appendChild(ring);
ring.classList.add('ok');
var fg = ring.querySelector('.aaa-tr-fg');
var r = 27; var circ = 2 * Math.PI * r;
fg.style.strokeDasharray = circ.toFixed(2);
fg.style.strokeDashoffset = '0';
ring.dataset.circ = circ.toFixed(2);
}
function aaaUpdateTimerRing() {
var ring = document.getElementById('aaaTimerRing');
var txt = document.getElementById('multiEnemyTimerDisplay');
if (!ring) return;
var sec = 10;
if (txt) { var mm = (txt.textContent || '').match(/([0-9.]+)/); if (mm) sec = parseFloat(mm[1]); }
else if (typeof multiEnemyTimeLeft !== 'undefined') sec = multiEnemyTimeLeft;
sec = Math.max(0, Math.min(10, sec));
var circ = parseFloat(ring.dataset.circ) || (2 * Math.PI * 27);
var fg = ring.querySelector('.aaa-tr-fg');
if (fg) fg.style.strokeDashoffset = (circ * (1 - sec / 10)).toFixed(2);
var num = ring.querySelector('.aaa-tr-num');
if (num) num.textContent = sec.toFixed(1).replace(/\.0$/, '');
ring.classList.toggle('aaa-urgent', sec <= 3);
}
function aaaUpdateHpRatio() {
var fill = document.querySelector('.multi-boss-hp-fill');
if (!fill) return;
var ratio = 1;
if (typeof multiBossHp !== 'undefined' && typeof multiBossMaxHp !== 'undefined' && multiBossMaxHp > 0) {
ratio = Math.max(0, Math.min(1, multiBossHp / multiBossMaxHp));
}
fill.style.setProperty('--hp-ratio', ratio.toFixed(3));
}

// ------------------------------------------------------------------
// 【3】既存関数のラップ（元を必ず呼ぶ＝ロジック温存）
// ------------------------------------------------------------------
// 3-a updateMultiHpBars：HP比率＋タイマー＋敵ステージを毎描画同期
if (typeof window.updateMultiHpBars === 'function' && !window.updateMultiHpBars.__aaaWrapped) {
var __origUpdateHp = window.updateMultiHpBars;
window.updateMultiHpBars = function() {
var r = __origUpdateHp.apply(this, arguments);
try { aaaUpdateHpRatio(); } catch (e) {}
try { aaaUpdateTimerRing(); } catch (e) {}
return r;
};
window.updateMultiHpBars.__aaaWrapped = true;
}
// 3-b handleMultiBattleTimer：タイマー円を毎ティック更新＋バー揺れ
if (typeof window.handleMultiBattleTimer === 'function' && !window.handleMultiBattleTimer.__aaaWrapped) {
var __origTimer = window.handleMultiBattleTimer;
window.handleMultiBattleTimer = function() {
var r = __origTimer.apply(this, arguments);
try { aaaUpdateTimerRing(); } catch (e) {}
try {
var bar = document.querySelector('.multi-boss-full-bar');
if (bar && typeof multiEnemyTimeLeft !== 'undefined' && multiEnemyTimeLeft <= 0.05) {
bar.classList.remove('aaa-bar-shake'); void bar.offsetWidth; bar.classList.add('aaa-bar-shake');
}
} catch (e) {}
return r;
};
window.handleMultiBattleTimer.__aaaWrapped = true;
}
// 3-c showCharacterPopup：攻撃にCRIT風装飾＋敵画像ヒット演出
if (typeof window.showCharacterPopup === 'function' && !window.showCharacterPopup.__aaaWrapped) {
var __origPopup = window.showCharacterPopup;
window.showCharacterPopup = function(memberId, amount, type) {
var r = __origPopup.apply(this, arguments);
try {
if (type === 'attack') {
var img = document.getElementById('multiBossImage');
if (img) { img.classList.remove('aaa-hit'); void img.offsetWidth; img.classList.add('aaa-hit'); }
// コンボに応じて直近の攻撃バブルをCRIT化
var crit = (typeof gameComboCount !== 'undefined' && gameComboCount >= 5);
if (crit) {
var bubbles = document.querySelectorAll('.popup-bubble-flying-atk:not(.aaa-crit)');
...
window.showCharacterPopup.__aaaWrapped = true;
}
// 3-d processMultiFlickAnswer：正解/不正解の選択肢フィードバック＋コンボバナー
if (typeof window.processMultiFlickAnswer === 'function' && !window.processMultiFlickAnswer.__aaaWrapped) {
var __origFlick = window.processMultiFlickAnswer;
window.processMultiFlickAnswer = function(choiceIndex) {
// 押した選択肢にフィードバッククラスを一時付与
try {
var el = document.getElementById('multiChoice-' + choiceIndex);
if (el) {
var ok = (choiceIndex === currentMultiCorrectIndex);
el.classList.add(ok ? 'aaa-correct' : 'aaa-wrong');
setTimeout(function() { el.classList.remove('aaa-correct', 'aaa-wrong'); }, 460);
}
} catch (e) {}
var beforeCombo = (typeof gameComboCount !== 'undefined') ? gameComboCount : 0;
var r = __origFlick.apply(this, arguments);
try {
var afterCombo = (typeof gameComboCount !== 'undefined') ? gameComboCount : 0;
if (afterCombo > beforeCombo && afterCombo >= 2) aaaShowCombo(afterCombo);
else if (afterCombo === 0 && beforeCombo >= 2) aaaHideCombo();
} catch (e) {}
return r;
};
window.processMultiFlickAnswer.__aaaWrapped = true;
}
// コンボバナー制御
var __aaaComboHideT = null;
function aaaComboTier(c) { if (c >= 20) return 4; if (c >= 10) return 3; if (c >= 5) return 2; return 1; }
function aaaComboWord(t) { return ['','COMBO','GREAT','EXCELLENT','UNSTOPPABLE'][t] || 'COMBO'; }
function aaaShowCombo(c) {
var b = document.getElementById('aaaComboBanner');
var f = document.getElementById('aaaComboFlash');
if (!b) return;
var t = aaaComboTier(c);
b.className = 'show tier-' + t;
b.querySelector('.aaa-cb-word').textContent = aaaComboWord(t);
b.querySelector('.aaa-cb-num').textContent = c + ' HITS';
if (f) f.className = 'tier-' + t;
clearTimeout(__aaaComboHideT);
__aaaComboHideT = setTimeout(aaaHideCombo, 1400);
}
function aaaHideCombo() {
var b = document.getElementById('aaaComboBanner');
var f = document.getElementById('aaaComboFlash');
...
// ------------------------------------------------------------------
// 【4】startMultiBattlePlay ラップ：戦闘開始時にDOMを組み直す
// ------------------------------------------------------------------
if (typeof window.startMultiBattlePlay === 'function' && !window.startMultiBattlePlay.__aaaWrapped) {
var __origStart = window.startMultiBattlePlay;
window.startMultiBattlePlay = function() {
var r = __origStart.apply(this, arguments);
setTimeout(function() {
try { aaaEnsureFx(); } catch (e) {}
try { aaaBuildEnemyStage(); } catch (e) {}
try { aaaBuildTimerRing(); } catch (e) {}
try { aaaUpdateHpRatio(); } catch (e) {}
try { aaaUpdateTimerRing(); } catch (e) {}
}, 60);
return r;
};
window.startMultiBattlePlay.__aaaWrapped = true;
}
// 戦闘終了時にFx/バナーを片付ける（cancelMultiBattlePlay ラップ）
if (typeof window.cancelMultiBattlePlay === 'function' && !window.cancelMultiBattlePlay.__aaaWrapped) {
var __origCancel = window.cancelMultiBattlePlay;
window.cancelMultiBattlePlay = function() {
try { aaaHideCombo(); } catch (e) {}
var r = __origCancel.apply(this, arguments);
return r;
};
window.cancelMultiBattlePlay.__aaaWrapped = true;
}

// ------------------------------------------------------------------
// 【5】起動時：既に戦闘画面が開いている場合の保険
// ------------------------------------------------------------------
(function aaaBoot() {
function run() {
var play = document.getElementById('multi-battle-play-screen');
if (play && play.style.display === 'flex') {
aaaEnsureFx(); aaaBuildEnemyStage(); aaaBuildTimerRing(); aaaUpdateHpRatio(); aaaUpdateTimerRing();
}
}
...
console.log('🎮 AAA戦闘UI全面リデザインパッチ適用完了（CSS主導・ロジック温存）');
})();
// ==========================================================================
// 🔧 ガシャ管理行 修理パッチ（ネスト塊を除去してフラットな admRow で再注入）
//    fix.js の末尾にそのまま追記。既存コードは一切変更不要。
// ==========================================================================
(function () {
"use strict";
if (window.__gxAdminFixApplied) return;
window.__gxAdminFixApplied = true;
window.__gachaAdminApplied = true; // 旧ブロックの起動ガードを先に潰す

var TITLES = ['ゴールド', 'ガチャチケット', 'カケラ', 'キャラ（所持と強化Lv）', 'アイテム（武器・防具の所持）'];
...
/* ---- 壊れたネスト塊を除去（自分の行 data-gx は残す） ---- */
function cleanBroken() {
  var list = document.getElementById('admList');
  if (!list) return;
  var rows = list.querySelectorAll('.admRow');
  for (var i = 0; i < rows.length; i++) {
    var r = rows[i];
    if (r.getAttribute('data-gx')) continue; // 自分の行は残す
    var nm = r.querySelector('.admName');
    var txt = nm ? nm.textContent : '';
    for (var t = 0; t < TITLES.length; t++) {
      if (txt.indexOf(TITLES[t]) === 0) { if (r.parentNode) r.parentNode.removeChild(r); break; }
    }
  }
}

/* ---- フラットな行ビルダー（閉じタグ完全対応） ---- */
function row(icon, title, nowTxt, desc, editHtml) {
  return '<div class="admRow in" data-gx="1">' +
    '<div class="admTop">' +
    '<div class="admIco">' + icon + '</div>' +
    '<div class="admBody">' +
    '<div class="admName">' + esc(title) + '</div>' +
    '<div class="admNow"><span class="tick"></span><span class="val">' + esc(nowTxt) + '</span></div>' +
    '<div class="admDesc">' + esc(desc) + '</div>' +
    '</div>' +
    '<div class="admActs"><button class="admBtn edit" type="button">編集</button><button class="admBtn reset" type="button">リセット</button></div>' +
    '</div>' +
    ...
    '<div class="admConfirm"><span>本当にリセットしますか？</span><button class="admNo" type="button">やめる</button><button class="admYes" type="button">リセット</button></div>' +
    '</div>';
}
function line(input, unit) { return '<div class="line">' + input + '<span class="unit">' + esc(unit) + '</span></div>'; }
function numIn(id, v) { return '<input type="number" data-f="' + id + '" value="' + v + '">'; }

function buildAll() {
  var h = '';
  h += row('🪙', 'ゴールド', String(gget('gold', 0)), 'バトル報酬・ログインボーナスで増える', line(numIn('gold', gget('gold', 0)), 'ゴールド'));
  h += row('🎟️', 'ガチャチケット',
    '共 ' + gget('gacha_tickets', 0) + ' / キャラ ' + gget('gacha_ticket_char', 0) + ' / アイテム ' + gget('gacha_ticket_item', 0),
    '共通／キャラ／アイテムの3種',
    line(numIn('tc', gget('gacha_tickets', 0)), '共通') + line(numIn('tch', gget('gacha_ticket_char', 0)), 'キャラ') + line(numIn('ti', gget('gacha_ticket_item', 0)), 'アイテム'));
  h += row('💎', 'カケラ',
    'C ' + gget('gacha_shard_C', 0) + ' / UC ' + gget('gacha_shard_UC', 0) + ' / R ' + gget('gacha_shard_R', 0) + ' / SR ' + gget('gacha_shard_SR', 0),
    '強化素材（レア度別）',
    line(numIn('sc', gget('gacha_shard_C', 0)), 'C') + line(numIn('suc', gget('gacha_shard_UC', 0)), 'UC') + line(numIn('sr', gget('gacha_shard_R', 0)), 'R') + line(numIn('ssr', gget('gacha_shard_SR', 0)), 'SR'));
  h += row('🐧', 'キャラ（所持と強化Lv）',
    '所持 ' + (gget('gacha_inv_char', []) || []).length + '体',
    '所持はIDをカンマ区切り／強化は id:lv をカンマ区切り',
    line('<input type="text" data-f="ownc" value="' + esc((gget('gacha_inv_char', []) || []).join(',')) + '">', '所持ID') +
    line('<input type="text" data-f="lv" value="' + esc(Object.keys(gget('gacha_enhance', {}) || {}).map(function (k) { return k + ':' + (gget('gacha_enhance', {}) || {})[k]; }).join(',')) + '">', '強化 id:lv'));
  h += row('🗡️', 'アイテム（武器・防具の所持）',
    '武器 ' + (gget('gacha_inv_weapon', []) || []).length + ' / 防具 ' + (gget('gacha_inv_armor', []) || []).length,
    '所持IDをカンマ区切りで入力',
    line('<input type="text" data-f="ownw" value="' + esc((gget('gacha_inv_weapon', []) || []).join(',')) + '">', '武器ID') +
    line('<input type="text" data-f="owna" value="' + esc((gget('gacha_inv_armor', []) || []).join(',')) + '">', '防具ID'));
  return h;
}
function splitIds(v) { return String(v || '').split(',').map(function (x) { return x.trim(); }).filter(function (x) { return x !== ''; }); }

function saveRow(rowEl) {
  var name = rowEl.querySelector('.admName').textContent;
  function num(f) { var i = rowEl.querySelector('[data-f="' + f + '"]'); return i ? (parseInt(i.value, 10) || 0) : 0; }
  function txt(f) { var i = rowEl.querySelector('[data-f="' + f + '"]'); return i ? i.value : ''; }
  if (name.indexOf('ゴールド') === 0) { gset('gold', num('gold')); }
  else if (name.indexOf('ガチャチケット') === 0) { gset('gacha_tickets', num('tc')); gset('gacha_ticket_char', num('tch')); gset('gacha_ticket_item', num('ti')); }
  else if (name.indexOf('カケラ') === 0) { gset('gacha_shard_C', num('sc')); gset('gacha_shard_UC', num('suc')); gset('gacha_shard_R', num('sr')); gset('gacha_shard_SR', num('ssr')); }
  else if (name.indexOf('キャラ') === 0) {
    gset('gacha_inv_char', splitIds(txt('ownc')));
    var lv = {}; splitIds(txt('lv')).forEach(function (kv) { var p = kv.split(':'); if (p.length === 2) lv[p[0]] = parseInt(p[1], 10) || 0; });
    gset('gacha_enhance', lv);
  }
  else if (name.indexOf('アイテム') === 0) { gset('gacha_inv_weapon', splitIds(txt('ownw'))); gset('gacha_inv_armor', splitIds(txt('owna'))); }
  saveAll();
}
function resetRow(rowEl) {
  var name = rowEl.querySelector('.admName').textContent;
  if (name.indexOf('ゴールド') === 0) gset('gold', 0);
  else if (name.indexOf('ガチャチケット') === 0) { gset('gacha_tickets', 0); gset('gacha_ticket_char', 0); gset('gacha_ticket_item', 0); }
  else if (name.indexOf('カケラ') === 0) { gset('gacha_shard_C', 0); gset('gacha_shard_UC', 0); gset('gacha_shard_R', 0); gset('gacha_shard_SR', 0); }
  else if (name.indexOf('キャラ') === 0) { gset('gacha_inv_char', []); gset('gacha_enhance', {}); }
  else if (name.indexOf('アイテム') === 0) { gset('gacha_inv_weapon', []); gset('gacha_inv_armor', []); }
  saveAll();
}
...
function inject() {
  ...
  if (!list) return;
  cleanBroken();
  if (list.querySelector('[data-gx]')) return; // 既に正しい行がある
  var wrap = document.createElement('div');
  wrap.setAttribute('data-gx-wrap', '1');
  wrap.innerHTML = buildAll();
  // フラットに（wrap を使わず直接 list へ）
  while (wrap.firstChild) list.appendChild(wrap.firstChild);
  // イベント束ね
  var rows = list.querySelectorAll('[data-gx]');
  for (var i = 0; i < rows.length; i++) {
    (function (r) {
      var ed = r.querySelector('.admBtn.edit'), rs = r.querySelector('.admBtn.reset');
      var bk = r.querySelector('.admEdit .back'), go = r.querySelector('.admEdit .go');
      var no = r.querySelector('.admConfirm .admNo'), yes = r.querySelector('.admConfirm .admYes');
      if (ed) ed.onclick = function () { r.classList.remove('asking'); r.classList.add('editing'); };
      if (rs) rs.onclick = function () { r.classList.remove('editing'); r.classList.add('asking'); };
      if (bk) bk.onclick = function () { r.classList.remove('editing'); };
      if (go) go.onclick = function () { saveRow(r); r.classList.remove('editing'); refresh(); };
      if (no) no.onclick = function () { r.classList.remove('asking'); };
      if (yes) yes.onclick = function () { resetRow(r); r.classList.remove('asking'); refresh(); };
    })(rows[i]);
  }
}
function refresh() {
  var list = document.getElementById('admList');
  if (!list) return;
  var old = list.querySelectorAll('[data-gx]');
...
if (typeof MutationObserver !== 'undefined') {
  new MutationObserver(function () { cleanBroken(); inject(); }).observe(document.body, { childList: true, subtree: true });
}
setInterval(function () { cleanBroken(); inject(); }, 800);
console.log('🔧 ガシャ管理行 修理パッチ適用完了');
})();
// ==========================================================================
// 🗑️ 既存「全ユーザー一括消去」ダイアログへ“選択式”を組み込むパッチ
//    ・新しいボタン/パネルは増やさない。既存ダイアログ(waOverlay)の中に
//      「消す項目」チェックを注入し、実行ボタンを選択式に差し替えるだけ
//    ・全員(クラウド)＋自分(ローカル)＋他端末(次回起動時)を選択した分だけ消去
//    ※ fix.js / app.js は不変更。末尾追記のみ
// ==========================================================================
(function applyWipeSelectMergePatch(){
"use strict";
if (window.__wipeSelectMerged) return;
...
var CATS = [
 ...
 {id:'gacha', label:'ガチャデータ'},
 {id:'friend',label:'フレンド'}
];
function has(cats,id){ return cats.indexOf(id)>=0; }

/* ---------- 既存ダイアログへ選択チェックを注入 ---------- */
function injectSelect(){
 var ov = document.getElementById('waOverlay');
 if (!ov || ov.querySelector('#waSelectBox')) return;
 var card = ov.querySelector('.wa-card');
 var actions = ov.querySelector('#waActions');
 if (!card) return;
 var box = document.createElement('div');
 box.id = 'waSelectBox';
 box.style.cssText = 'margin:10px 0 12px;display:flex;flex-wrap:wrap;gap:6px;';
 var html = '<div style="width:100%;font-size:11px;font-weight:800;color:#fca5a5;margin-bottom:2px;">🗑 消す項目（チェックしたものだけ消去）</div>';
 CATS.forEach(function(c){
   html += '<label style="display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;color:#e2e8f0;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:5px 10px;cursor:pointer;"><input type="checkbox" data-wcat="'+c.id+'" checked style="width:14px;height:14px;">'+c.label+'</label>';
 });
 box.innerHTML = html;
 if (actions) card.insertBefore(box, actions); else card.appendChild(box);
 // 固定の「消えるもの」チップは選択式に置き換えたので隠す
 var w = ov.querySelector('.wa-wipe'); if (w) w.style.display='none';
 // 実行ボタンを選択式に差し替え（2段階確認は維持）
 var yes = ov.querySelector('#waYes');
 if (yes) { yes.onclick = function(){ startSelective(ov); }; }
}
function getChecked(ov){
 var out=[]; var boxes = ov.querySelectorAll('[data-wcat]');
 for (var i=0;i<boxes.length;i++) if (boxes[i].checked) out.push(boxes[i].getAttribute('data-wcat'));
 return out;
}

/* ---------- 2段階→選択式消去 ---------- */
function startSelective(ov){
 if (ov.dataset.busy) return;
 if (ov.dataset.step !== '2') {
   ov.dataset.step='2';
   var yes=ov.querySelector('#waYes'); if(yes){ yes.textContent='本当に実行'; yes.classList.add('final'); }
   return;
 }
 ov.dataset.busy='1';
 var cats = getChecked(ov);
 if (!cats.length) { ov.dataset.busy=''; ov.dataset.step=''; alert('消す項目を選択してください'); return; }
 var prog=ov.querySelector('#waProgress'), fill=ov.querySelector('#waFill'), ptext=ov.querySelector('#waPtext');
 if (prog) prog.classList.add('show');
 if (ptext) ptext.textContent='消去中…';
 wipeSharedSelective(cats)
 .then(function(){ return wipeAllUsersSelective(cats, function(done,total){ if(fill) fill.style.width=Math.round(done/total*90)+'%'; if(ptext) ptext.textContent='処理中 '+done+'/'+total; }); })
 .then(function(){ wipeLocalSelective(cats); bumpSelGen(cats); if(fill) fill.style.width='100%'; if(ptext) ptext.textContent='完了'; })
 .then(function(){
   setTimeout(function(){
     if(prog) prog.classList.remove('show');
     var doneBox=ov.querySelector('#waDone'); if(doneBox) doneBox.classList.add('show');
     setTimeout(function(){ ov.classList.remove('open'); ov.dataset.busy=''; ov.dataset.step=''; var y=ov.querySelector('#waYes'); if(y){y.textContent='実行する';y.classList.remove('final');} var d=ov.querySelector('#waDone'); if(d)d.classList.remove('show'); }, 1500);
   },300);
 }).catch(function(){ if(ptext) ptext.textContent='一部失敗'; ov.dataset.busy=''; });
}

/* ---------- 共有(ランキング等) ---------- */
function wipeSharedSelective(cats){
 var ps=[]; if(!window.db||!window.fbSetDoc||!window.fbDoc) return Promise.resolve();
 if(has(cats,'game')){
  ['ja2en','en2ja','mixed'].forEach(function(m){
    ps.push(window.fbSetDoc(window.fbDoc(window.db,'shared','game_hall_'+m),{scores:[]},{merge:true}).catch(function(){}));
    ['endless','normal','hard','expert'].forEach(function(d){ ps.push(window.fbSetDoc(window.fbDoc(window.db,'shared','game_hall_'+m+'_'+d),{scores:[]},{merge:true}).catch(function(){})); });
  });
  if (window.getCurrentSeasonNo){ var cur=window.getCurrentSeasonNo()||0; for(var n=1;n<=cur;n++){ var mm=(window.getSeasonMode)?window.getSeasonMode(n):'ja2en'; ps.push(window.fbSetDoc(window.fbDoc(window.db,'shared','game_season_'+n+'_'+mm),{scores:[]},{merge:true}).catch(function(){})); } }
 }
 return Promise.all(ps);
}
/* ---------- 全ユーザー(クラウド) ---------- */
function wipeAllUsersSelective(cats,onProgress){
 return window.getAllUsers().then(function(users){
  var ids=(users||[]).map(function(u){return u&&u.id;}).filter(function(id){return id&&id!=='GUEST-000';});
  if (typeof myId!=='undefined'&&myId&&myId!=='GUEST-000'&&ids.indexOf(myId)<0) ids.push(myId);
  var chain=Promise.resolve(); var done=0;
  ids.forEach(function(id){
    chain=chain.then(function(){
      return window.fbGetDoc(window.fbDoc(window.db,'users',id)).then(function(snap){
        if(!snap||!snap.exists()) return;
        var d=snap.data()||{}; d.userStats=d.userStats||{}; var ch=false;
        if(has(cats,'exp')){ d.totalExp=0; d.userStats.user_level=1; ch=true; }
        if(has(cats,'title')){ d.selectedTitle='称号なし'; d.userStats.seasonTitles=[]; d.userStats.settledSeasons=[]; ch=true; }
        if(has(cats,'study')){ d.userStats.study_total_secs=0; d.userStats.study_today_secs=0; d.userStats.study_week_secs=0; d.userStats.study_burst=0; ch=true; }
        if(has(cats,'gacha')){ d.userStats.gold=0; d.userStats.gacha_tickets=0; ['C','UC','R','SR'].forEach(function(r){d.userStats['gacha_shard_'+r]=0;}); d.userStats.gacha_inv_char=[]; d.userStats.gacha_inv_weapon=[]; d.userStats.gacha_inv_armor=[]; d.userStats.gacha_enhance={}; ch=true; }
        if(has(cats,'friend')){ d.friendList=[]; d.userStats.friends_count=0; ch=true; }
        var ps=[];
        if(ch) ps.push(window.fbSetDoc(window.fbDoc(window.db,'users',id), d, {merge:true}).catch(function(){}));
        if(has(cats,'exp')||has(cats,'title')) ps.push(window.fbSetDoc(window.fbDoc(window.db,'shared_leaderboard',id), {exp:(has(cats,'exp')?0:(d.totalExp||0)), level:(has(cats,'exp')?1:(d.userStats.user_level||1)), title:(has(cats,'title')?'称号なし':(d.selectedTitle||''))}, {merge:true}).catch(function(){}));
        if(has(cats,'vocab')&&window.fbDeleteDoc){
          var books=['default']; try{ if(typeof textbooksPool!=='undefined'&&textbooksPool) textbooksPool.forEach(function(b){books.push(b.id);}); }catch(e){}
          books.forEach(function(b){ ps.push(window.fbDeleteDoc(window.fbDoc(window.db,'users',id,'vocabProgress',b)).catch(function(){})); });
        }
        return Promise.all(ps);
      }).catch(function(){});
    }).then(function(){ done++; if(onProgress) onProgress(done, ids.length); });
  });
  return chain;
 });
}
/* ---------- 自分(ローカル) ---------- */
function wipeLocalSelective(cats){
 if(has(cats,'exp')){ try{ totalExp=0; localStorage.setItem('core_v4_totalExp','0'); userStats.user_level=1; }catch(e){} }
 if(has(cats,'title')){ try{ selectedTitle='称号なし'; localStorage.setItem('core_v4_userTitle','称号なし'); userStats.seasonTitles=[]; userStats.settledSeasons=[]; rewardedTitlesStepsCache={}; localStorage.setItem('core_v4_rewarded_titles_cache','{}'); }catch(e){} }
 if(has(cats,'study')){ try{ todayStudySeconds=0; localStorage.setItem('core_v4_study_today_secs','0'); weeklyStudyMinutesLog=[0,0,0,0,0,0,0]; localStorage.setItem('core_v4_study_weekly_log','[0,0,0,0,0,0,0]'); userStats.study_total_secs=0; userStats.study_burst=0; }catch(e){} }
 if(has(cats,'vocab')){ try{ wordMemory={}; localStorage.setItem('wordMemory','{}'); }catch(e){} }
 if(has(cats,'game')){ try{ ['ja2en','en2ja','mixed'].forEach(function(m){ ['endless','normal','hard','expert'].forEach(function(d){ localStorage.removeItem('cosmic_score_'+m+'_'+d); localStorage.removeItem('cosmic_best_'+m+'_'+d); }); }); }catch(e){} }
 if(has(cats,'gacha')){ try{ userStats.gold=0; userStats.gacha_tickets=0; ['C','UC','R','SR'].forEach(function(r){userStats['gacha_shard_'+r]=0;}); userStats.gacha_inv_char=[]; userStats.gacha_inv_weapon=[]; userStats.gacha_inv_armor=[]; userStats.gacha_enhance={}; }catch(e){} }
 if(has(cats,'friend')){ try{ myFriendList=[]; localStorage.setItem('core_v4_friend_list','[]'); userStats.friends_count=0; }catch(e){} }
 try{ localStorage.setItem('core_v4_user_stats_'+myId, JSON.stringify(userStats)); }catch(e){}
 try{ if(window.saveUserStats) window.saveUserStats(); }catch(e){}
 try{ if(window.renderLeaderboard) window.renderLeaderboard(); }catch(e){}
 try{ if(window.renderActivityChart) window.renderActivityChart(); }catch(e){}
}
/* ---------- 他端末(次回起動時)に選択分だけ消させる ---------- */
function bumpSelGen(cats){
 if(!window.db||!window.fbSetDoc||!window.fbDoc) return;
 try{ window.fbSetDoc(window.fbDoc(window.db,'shared','app_settings'), { selResetGen: Date.now(), selResetCats: cats }, {merge:true}).catch(function(){}); }catch(e){}
}
function applySelResetOnLoad(){
 if(!window.db||!window.fbGetDoc||!window.fbDoc) return;
 if(typeof myId==='undefined'||!myId||myId==='GUEST-000') return;
 window.fbGetDoc(window.fbDoc(window.db,'shared','app_settings')).then(function(snap){
   if(!snap||!snap.exists()) return;
   var d=snap.data()||{}; var gen=parseInt(d.selResetGen)||0; var cats=d.selResetCats||[];
   if(!gen||!cats.length) return;
   var key='__sel_reset_gen_'+myId; var local=parseInt(localStorage.getItem(key))||0;
   if(gen>local){ wipeLocalSelective(cats); try{ localStorage.setItem(key,String(gen)); }catch(e){} }
 }).catch(function(){});
}
/* ---------- 起動フック ---------- */
...
setInterval(function(){ var ov=document.getElementById('waOverlay'); if(ov&&ov.classList.contains('open')) injectSelect(); },300);
setTimeout(applySelResetOnLoad,1200);
console.log('🗑️ 既存一括消去ダイアログへ選択式組み込みパッチ 適用完了');
})();
